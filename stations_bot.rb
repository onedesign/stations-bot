module StationsBot
  class API < Grape::API
    ...
    helpers do
      def authenticate
        params['token'] == ENV['SLACK_APPLICATION_TOKEN'] || Team.where(access_token: params['token']).any?
      end
      ...
      def build_command
        if (args.nil? || args == '') && default = Place.where(user_id: user.id, name: 'default').first
